...
app.post('/api/analyze-mood', async (req, res) => {
  ...
  try {
    ...
    const secondResponse = await openai.completions.create({
      model: 'gpt-3.5-turbo-instruct',
      prompt: `I will provide some text to you. Analyze the text to determine an overall emotional state. Respond in a single word that describes the mood that the text conveys. The text is: "${userText}"`,
      max_tokens: 40,
    });
